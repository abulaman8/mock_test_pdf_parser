...
html_content = ""
with open('html/Maths2.html', 'r', encoding='utf-8') as f:
    html_content = f.read()
...
sorted_courses = ['Maths2']
sorted_courses.append("end_of_paper")
# print(sorted_courses)
with open('new_json/maths2.html', 'w', encoding='utf-8') as f:
    f.write(full_soup.prettify())
        ...
        if "COMPREHENSION" in tags[0].string:
            ...
            if "Question Type : SA" in tags[0].string:
                ...
                while start and start != end:
                    choice_chunk.append(str(start))
                    # print(str(start))
                    # print(start.string)
                    try:
                        i = int(start.string[:-1])
                        # print(f"i: {i}")
                        if i > 100000000:
                            choice_ids.append(start.string)
                            # print(choice_ids)
                        ...
                        pass
                    start = start.next_sibling
                # print(len(choice_chunk))
                choice_chunk = BeautifulSoup("".join(choice_chunk), 'html.parser')
                choice_ids.append(BeautifulSoup('<p>end</p>', 'html.parser'))
                all_choices = []
                # print(f"choices ids: {choice_ids}")
                for i in range(len(choice_ids)-1):
                    choice = []
                    # print(choice_ids[i])
                    # print(choice_ids[i+1])
                    start = choice_chunk.find(
                        "p",
                        string=choice_ids[i]
                    )
                    # print(start)
                    end = choice_chunk.find(
                        "p",
                        string=choice_ids[i+1]
                    )
                    # print(end)
                    while True:
                        if start and start != end:
                            choice.append(str(start))
                            start = start.next_sibling
                        else:
                            all_choices.append(BeautifulSoup("".join(choice), "html.parser"))
                            break
                ...
                for c in all_choices:
                    ...
                    # print(id.string)
                    choice["id"] = id.string
                    bool_img = id.next_sibling
                    if bool_img["src"] == correct:
                        choice["is_correct"] = True
                    else:
                        choice["is_correct"] = False
                    content = bool_img.next_sibling
                    if not content:
                        break
                        # print(choice_ids)
                    ...
                    if content.name == "img":
                        ...
                        while True:
                            if content and content != end:
                                cl.append(str(content))
                                # print(f"len of all choices: {len(all_choices)}")
                                # print('------------------------------------------')
                                # print(str(start)[:20])
                                # print(c.prettify())
                                # exit(1)
...
for c in ccd:
    with open(f"new_json/{c}.json", "w", encoding="utf-8") as f:
