...
strings = full_soup.find_all(string=True)
start = full_soup.find("p", string="Question Paper Name :").next_sibling
end = full_soup.find("p", string="Subject Name :")
question_paper_name = ""
while start != end:
...
for course in ccd:
    ...
    for qn in ccd[course]["questions"]:
        ...
        course_qns.append(q)
        all_qns.append(q)
    qp.questions = course_qns
    all_qps.append(qp)
    for comp in ccd[course]["comprehensions"]:
        comp_images = []
        comp_qn_nos = comp["qns"]
        new_comp = Comprehension(
                text=comp["content"],
                question_paper=qp
                )
        if len(comp["images"]):
            for img in comp["images"]:
                ...
                comp_images.append(new_img)
        new_comp.images = comp_images
        all_images.extend(comp_images)
        for qn in course_qns:
            if qn.qn_no in comp_qn_nos:
                new_comp.questions.append(qn)
        course_comprehensions.append(new_comp)
        qp.comprehensions.append(new_comp)
        all_comprehensions.append(new_comp)
